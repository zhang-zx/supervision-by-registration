...
def load_video_dir(root, dirs, save_dir, save_name):
    ...
    for idx, cdir in enumerate(dirs):
        ...
        assert len(all_frames) == len(all_annots), 'The length is not right for {} : {} vs {}'.format(cdir,
                                                                                                      len(all_frames),
                                                                                                      len(all_annots))
        ...
        print(
            '--->>> {:} : [{:02d}/{:02d}] : {:} has {:} frames | face size : mean={:.2f}, std={:.2f}'.format(save_name,
                                                                                                             idx,
                                                                                                             len(dirs),
                                                                                                             cdir, len(
                    all_frames), all_sizes.mean(), all_sizes.std()))
...
def generate_300vw_list(root, save_dir):
    ...
    test_1_dirs = [114, 124, 125, 126, 150, 158, 401, 402, 505, 506, 507, 508, 509, 510, 511, 514, 515, 518, 519, 520,
                   521, 522, 524, 525, 537, 538, 540, 541, 546, 547, 548]
    ...
    train_dirs = ['009', '059', '002', '033', '020', '035', '018', '119', '120', '025', '205', '047', '007', '013',
                  '004', '143',
                  '034', '028', '053', '225', '041', '010', '031', '046', '049', '011', '027', '003', '016', '160',
                  '113', '001', '029', '043',
                  '112', '138', '144', '204', '057', '015', '044', '048', '017', '115', '223', '037', '123', '019',
                  '039', '022']

    test_1_dirs, test_2_dirs, test_3_dirs = ['{}'.format(x) for x in test_1_dirs], ['{}'.format(x) for x in
                                                                                    test_2_dirs], ['{}'.format(x) for x
                                                                                                   in test_3_dirs]
