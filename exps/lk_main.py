...
def main(args):
    ...
    # Training Dataset
    train_data = VDataset(train_transform, args.sigma, model_config.downsample, args.heatmap_type, args.data_indicator,
                          args.video_parser)
    ...
    if args.eval_vlists is not None:
        for eval_vlist in args.eval_vlists:
            eval_vdata = IDataset(eval_transform, args.sigma, model_config.downsample, args.heatmap_type,
                                  args.data_indicator)
    ...
    if args.eval_ilists is not None:
        for eval_ilist in args.eval_ilists:
            eval_idata = IDataset(eval_transform, args.sigma, model_config.downsample, args.heatmap_type,
                                  args.data_indicator)
    ...
    for epoch in range(start_epoch, opt_config.epochs):
        ...
        # train for one epoch
        train_loss = train(args, train_loader, net, criterion, optimizer, epoch_str, logger, opt_config, lk_config,
