  ...
  '''
    ...
    for point_index in range(n_points):
        try:
            ...
            pts[2, point_index] = float(
                1)  # oculusion flag, 0: oculuded, 1: visible. We use 1 for all points since no visibility is provided by 300-W
...
def convert68to49(points):
    points = points.copy()
    assert len(points.shape) == 2 and (points.shape[0] == 3 or points.shape[0] == 2) and points.shape[
        1] == 68, 'The shape of points is not right : {}'.format(points.shape)
