...
def lk_input_check(batch_locs, batch_scos, batch_next, batch_fback, batch_back):
    ...
    assert batch_locs.size() == batch_next.size() == batch_fback.size() == batch_back.size(), '{:} vs {:} vs {:} vs {:}'.format(
        batch_locs.size(), batch_next.size(), batch_fback.size(), batch_back.size())
    assert _ == 2, '{:}'.format(batch_locs.size())
    assert batch_scos.size(0) == batch and batch_scos.size(1) == sequence and batch_scos.size(
        2) == num_pts, '{:} vs {:}'.format(batch_locs.size(), batch_scos.size())
...
def lk_target_loss(batch_locs, batch_scos, batch_next, batch_fbak, batch_back, lk_config, video_or_not, mask, nopoints):
            ...
            for iseq in range(sequence):
                ...
                if iseq > 0:
                    if lk_config.stable:
                        ...
                        loss += torch.dist(targets, backPts)
                if iseq + 1 < sequence:
                    if lk_config.stable:
                        loss += torch.dist(targets, nextPts.detach())
                    else:
                        loss += torch.dist(targets, nextPts)
    ...
    if avaliable == 0:
        return None, avaliable
    else:
        return torch.mean(torch.stack(losses)), avaliable
