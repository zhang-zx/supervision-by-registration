...
def lk_tensor_track(feature_old, feature_new, pts_locations, patch_size, max_step, threshold=0.0001):
    ...
    assert feature_old.dim() == 3 and feature_new.dim() == 3, 'The dimension is not right : {} and {}'.format(
        feature_old.dim(), feature_new.dim())
    C, H, W = feature_old.size(0), feature_old.size(1), feature_old.size(2)
    assert C == feature_new.size(0) and H == feature_new.size(1) and W == feature_new.size(
        2), 'The size is not right : {}'.format(feature_new.size())
    assert pts_locations.dim() == 1 and pts_locations.size(0) == 2, 'The location is not right : {}'.format(
        pts_locations)
    if isinstance(patch_size, int): patch_size = (patch_size, patch_size)
    assert isinstance(patch_size, tuple) and len(patch_size) == 2 and isinstance(max_step,
                                                                                 int), 'The format of lk-parameters are not right : {}, {}'.format(
        patch_size, max_step)
    assert isinstance(patch_size[0], int) and isinstance(patch_size[1],
                                                         int), 'The format of lk-parameters are not right : {}'.format(
        patch_size)
