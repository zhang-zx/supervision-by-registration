...
def torch_inverse(deltp):
    assert deltp.dim() == 2 and deltp.size(0) == 2 and deltp.size(1) == 2, 'The deltp format is not right : {}'.format(
        deltp.size())
...
def ComputeGradient(feature, tag):
    ...
    if squeeze:
        return sobel(feature).squeeze(0)
    else:
        return sobel(feature)
...
def warp_feature(feature, pts_location, patch_size):
    ...
    crop_box = [pts_location[0] - patch_size[1], pts_location[1] - patch_size[0], pts_location[0] + patch_size[1],
                pts_location[1] + patch_size[0]]
    ...
    affine_parameter = [(crop_box[2] - crop_box[0]) / 2, MU.np2variable(torch.zeros(1), feature.is_cuda, False),
                        (crop_box[0] + crop_box[2]) / 2,
                        MU.np2variable(torch.zeros(1), feature.is_cuda, False), (crop_box[3] - crop_box[1]) / 2,
