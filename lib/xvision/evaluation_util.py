...
class Eval_Meta():
    ...
    def append(self, _pred, _ground, image_path, face_size):
        assert _pred.shape[0] == 3 and len(
            _pred.shape) == 2, 'Prediction\'s shape is {:} vs [should be (3,pts) or (2,pts)]'.format(_pred.shape)
        ...
        if (not self.predictions) == False:
            assert _pred.shape == self.predictions[-1].shape, 'shapes must be the same : {} vs {}'.format(_pred.shape,
                                                                                                          self.predictions[
                                                                                                              -1].shape)
    ...
    def compute_mse(self, log):
        ...
        if num == 0:
            ...
            nme, auc, pck_curves = evaluate_normalized_mean_error(self.predictions, self.groundtruth, log,
                                                                  self.face_sizes)
