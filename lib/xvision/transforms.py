...
class TrainScale2WH(object):
  ...
  """
    ...
    def __init__(self, target_size, interpolation=Image.BILINEAR):
        assert isinstance(target_size, tuple) or isinstance(target_size,
                                                            list), 'The type of target_size is not right : {}'.format(
            target_size)
        assert len(target_size) == 2, 'The length of target_size is not right : {}'.format(target_size)
        assert isinstance(target_size[0], int) and isinstance(target_size[1],
                                                              int), 'The type of target_size is not right : {}'.format(
            target_size)
...
class PreCrop(object):
  ...
  """
    ...
    def __init__(self, expand_ratio):
        assert expand_ratio is None or isinstance(expand_ratio,
                                                  numbers.Number), 'The expand_ratio should not be {}'.format(
            expand_ratio)
...
class AugScale(object):
  ...
  """
    ...
    def __init__(self, scale_prob, scale_min, scale_max, interpolation=Image.BILINEAR):
        ...
        assert isinstance(scale_min, numbers.Number) and isinstance(scale_max,
                                                                    numbers.Number), 'scales : {:}, {:}'.format(
            scale_min, scale_max)
