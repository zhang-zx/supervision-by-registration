...
def generate_label_map(pts, height, width, sigma, downsample, nopoints, ctype):
    # if isinstance(pts, numbers.Number): this image does not provide the annotation, pts is a int number
    # representing the number of points return np.zeros((height,width,pts+1), dtype='float32'), np.ones((1,1,1+pts),
    # dtype='float32') nopoints == True means this image does not provide the annotation, pts is a int number
